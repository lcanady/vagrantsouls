...
import { combatDefend } from '../../api/combat.ts';
...
export const customIdPrefix = 'combat:defend';
...
export async function execute(
  ...
  try {
    const result = await combatDefend(account.game_token, session.kvAdventurerId, {
    ...
    if (resolution) {
      ...
      await interaction.followUp({ content: 'Defend submitted...', ephemeral: true });
    }
  } catch (err) {
    await interaction.followUp({
      embeds: [buildErrorEmbed(`Defend failed: ${(err as Error).message}`)],
      ephemeral: true,
    });
  }
}
