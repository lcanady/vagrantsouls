...
import { buildQuestLogEmbed } from '../embeds/questlog.ts';
import { buildErrorEmbed } from '../embeds/error.ts';

export const data = new SlashCommandBuilder()
  .setName('quests')
  .setDescription('View your World Builder quest log.');
...
export async function execute(
  ...
  try {
    ...
    await interaction.editReply({ embeds: [buildQuestLogEmbed(adv)] });
  } catch (err) {
    await interaction.editReply({
